...
std::map<int, exercise> exam::list_dir(void)
{
    ...
    std::string path;
    // Determine a valid level directory to list exercises from
    int lvl_for_path = level;
    auto build_path = [&](int lvl) {
        if (student)
            return std::string(".subjects/STUD_PART/exam_0") + std::to_string(exam_number) + "/" + std::to_string(lvl) + "/";
        else
            return std::string(".subjects/PISCINE_PART/exam_0") + std::to_string(exam_number) + "/" + std::to_string(lvl) + "/";
    };
    path = build_path(lvl_for_path);
    DIR *dir = opendir(path.c_str());
    // If current level directory doesn't exist, fallback to the highest available below it
    while (dir == NULL && lvl_for_path > 0)
    {
        lvl_for_path--;
        path = build_path(lvl_for_path);
        dir = opendir(path.c_str());
    }
    // As a last resort, try level 0
    if (dir == NULL)
    {
        lvl_for_path = 0;
        path = build_path(lvl_for_path);
    ...
    if (dir == NULL)
    {
        std::cout << "Error: can't open any directory for exercises" << std::endl;
        sleep(100);
        return list;
    }
    while ((entry = readdir(dir)) != NULL)
    {
        ...
        {
            // Store the actual level used for this exercise list
            list.insert(std::pair<int, exercise>(i, exercise(lvl_for_path, folder)));
            i++;
        }
    }
    closedir(dir);
...
std::string exam::get_path(void)
{
    ...
    // Use the exercise's own stored level to build the path (may differ from current 'level' if clamped)
    int lvl_for_path = current_ex ? current_ex->get_lvl() : level;
    if (student)
        path_exam = ".subjects/STUD_PART/exam_0" + std::to_string(exam_number) + "/" + std::to_string(lvl_for_path) + "/" + current_ex->get_name() + "/";
    else
        path_exam = ".subjects/PISCINE_PART/exam_0" + std::to_string(exam_number) + "/" + std::to_string(lvl_for_path) + "/" + current_ex->get_name() + "/";
    return (path_exam);
}
...
void exam::ask_param(void)
{
    ...
    set_max_lvl();
    // Fixed 10 points per successful exercise, target 100 total
    level_per_ex_save = 10;
    level_per_ex = level * level_per_ex_save;
