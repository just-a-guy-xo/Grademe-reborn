...
void exam::restore_data(void)
{
    ...
    if (backup.end_time > time(0))
    {
        ...
        if (answer == "1")
        {
            ...
            // Migrate scoring to fixed 10 pts per success if needed
            this->level_per_ex_save = 10;
            // Recompute current accumulated points from level count
            this->level_per_ex = backup.level * this->level_per_ex_save;
